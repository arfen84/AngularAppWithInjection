(function () {
    ...
    angular
        .module('app')
        .controller('HotelModalCtrl', HotelModalCtrl);

    HotelModalCtrl.$inject = ['$scope', '$modalInstance', 'data'];

    function RestModalCtrl($scope, $modalInstance, data) {
        $scope.data = data;
        ...
        $scope.restauracja = function () {
            var $lokal = 'restauracja';
            $email = $('input[name=email]').val();

            $.ajax({
                url: "/Home/Hotel",
                type: "POST",
                data: { email: $email, lokal: $lokal, iloscGosci: $iloscGosci, bezalkoholowe: $bezalkoholowe, alkohol: $alkohol, oczekiwania: $oczekiwania, uslugi: $uslugi, miejscowosc: $miejscowosc, termin: $termin, budzet: $budzet },
        ...
        $scope.nextStep = function () {
            ...
            if (nextNumber == 5) {
                ...
                $("input:checkbox[name=bezalkoholowe]:checked").each(function () {
                    $val = $val + " " + $(this).val();
                });

                $bezalkoholowe = $('input[name=bezalkoholowe2]').val() + $val;
                //alert($bezalkoholowe);
            }

            if (nextNumber == 6) {
                $("input:checkbox[name=alkohol]:checked").each(function () {
                    $val = $val + " " + $(this).val();
                });

                $alkohol = $('input[name=alkohol2]').val() + $val;
                //alert($alkohol);
            }
            if (nextNumber == 7) {
